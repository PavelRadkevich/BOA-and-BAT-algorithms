...
public class BOAwithLevyFlightAlgorithmOwnVersion {
    int populationSize;
    int dimension;
    int iterations;
    double p;
    ...
    BOAwithLevyFlightAlgorithmOwnVersion(int populationSize,
    ...
    private void updateFragrance() {
        for (int i = 0; i < populationSize; i++) {
            population[i].fragrance = c * (Math.pow(population[i].fitness, a));
        }
    }
    ...
    private void updateBest() {
        ...
        double temp = 0;
        for (int i = 0; i < populationSize; i++) {
            double diff = population[i].previousFitness - population[i].fitness;
            if (diff > 0 && diff > temp) {
        ...
        if (bestButterfly == null) updateBestClassic();
    }

    private void updateBestClassic() {
    ...
    void updateFitness() {
        for (int i = 0; i < populationSize; i++) {
            population[i].fitness = f.count(population[i].position);
        }
    }
